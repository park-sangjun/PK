package com.spring.oop.encap;

public class Car {
    final String model; //모델명
    private int currentSpeed; //현재속도
    private int limitSpeed; //최대제한속도

    Car(String model, int limitSpeed){
        this.model = model;
//        this.currentSpeed = 0;  //안써도 됨
        this.limitSpeed = limitSpeed;

    }
    public void on(){
        injectOil();
        injectGas();
        moveCylinder();
        powerOn();
    }
    private void powerOn(){
        System.out.println("시동을 겁니다.");
    }
    private void injectOil(){
        System.out.println("엔진오일분사");
    }
    private void injectGas(){
        System.out.println("가솔린분사");
    }
    private void moveCylinder(){
        System.out.println("실린더작동");
    }
    public void accelerator(){
        System.out.println("악셓을밟았습니다.");
        if (this.currentSpeed >= 300) {
            System.out.println("이제못밟아");
            return;
        }
        this.currentSpeed +=50;
        System.out.println("현재속도 :"+currentSpeed);
    }
}
